...
{   
    ...
    //rules->setSelectionMode(QAbstractItemView::NoSelection);

    QDir d(label_path->text());
    setItems(d);

    QGridLayout *lay = new QGridLayout(this);
    lay->addWidget(label_path, 0, 0, 1, 2);
    lay->addWidget(dirs, 1, 0);
   // lay->addWidget(rules, 1, 1);
 ...
 //   QObject::connect(dirs, SIGNAL(QListView::verticalScrollbar()), rules, SLOT(QListView::verticalScrollbar()));
...
// если выкладываю, то указать авторство (по папке поиск)
void DirView::setItems(QDir dd){
    ...
    for(int i = 0; i < list.size(); i++){
        ...
        /*QString mode = "";
        if(f.isReadable()) mode += "r";
        else mode += "-";

        if(f.isWritable()) mode += "w";
        else mode += "-";

        if(f.isExecutable()) mode += "x";
        else mode += "-";

        QListWidgetItem *item = new QListWidgetItem(mode);
        item->setSizeHint(QSize(63, 63));
        rules->addItem(item);*/
    }
}
...
void DirView::contextMenu(QPoint pos){
    ...
    case 1:{
        ...
        if(ext == "tar" || ext == "zip" || ext == "7z" || ext == "gz"){
            ...
            QObject::connect(cut, SIGNAL(triggered(bool)), this, SLOT(cut()));
...
void DirView::open(){
    if(dirs->selectedItems().size() > 0){
        ...
        if(f.isFile()){
            ...
            else if(name == "zip" || name == "tar" || name == "rar") { // можно продолжить
                QProcess *text = new QProcess(this);
                text->start("/bin/file-roller", {f.absoluteFilePath()});
            }
            else{
                QProcess *text = new QProcess(this);
